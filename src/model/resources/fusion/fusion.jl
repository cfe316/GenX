...
function fusion_average_net_electric_power_expression!(EP::Model, inputs::Dict)
    ...
    net_th_frac[FUSION] .= active_frac[FUSION] .* (1 .- by_rid(FUSION,:Recirc_Act)) .- by_rid(FUSION,:Recirc_Pass) .- avg_start_power[FUSION]
...
function fusion_systemwide_max_cap_constraint!(EP::Model, inputs::Dict)
    ...
    if string(col) in names(dfTS)
        ...
        if max_cap >= 0
            @constraint(EP, cCSystemTot, sum(EP[:eCAvgNetElectric][FUSION]) <= max_cap)
        end
    end
end
...
function maximum_starts_constraint!(EP::Model,
    ...
    @constraint(EP, sum(start[y, t] * ω[t] for t in T) <=
...
function fusion_parasitic_power!(EP::Model,
        ...
        parasitic_start_power_factor::Float64,
    ...
    ePassive = EP[passive] = @expression(EP, [t in T],
                capacity[y] * eff_down * parasitic_passive_factor)

    # Active recirculating power, depending on committed capacity
    eActive = EP[active] = @expression(EP, [t in T],
                component_size * eff_down * parasitic_active_factor *
                (commit[y,t] - start[y,t] * dwell_time)
    )
    # Startup energy, taken from the grid every time the core starts up
    eStartEnergy = EP[start_energy] = @expression(EP, [t in T],
