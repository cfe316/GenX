...
function thermal_storage(EP::Model, setup::Dict, inputs::Dict)
	...
	if !isempty(FUS) #use fusion constraints if thermal cores tagged 'FUS' are present
		...
		@expression(EP, ePassiveRecircFus[y in FUS], vCCAP[y]*dfTS[dfTS.R_ID.==y,:Eff_Therm][]*dfTS[dfTS.R_ID.==y,:Recirc_Pass][]) #passive recirculating power, depending on built capacity
		@expression(EP, eActiveRecircFus[y in FUS, t=1:T], dfTS[dfTS.R_ID.==y,:Cap_Size][]*vFCOMMIT[y,t]*dfTS[dfTS.R_ID.==y,:Eff_Therm][]*dfTS[dfTS.R_ID.==y,:Recirc_Act][]) #active recirculating power, depending on committed capacity
		@expression(EP, eStartEnergyFus[y in FUS, t=1:T], dfTS[dfTS.R_ID.==y,:Cap_Size][]*vFSTART[y,t]*dfTS[dfTS.R_ID.==y,:Eff_Therm][]*dfTS[dfTS.R_ID.==y,:Start_Energy][]) #startup energy, taken from the grid every time the core starts up
