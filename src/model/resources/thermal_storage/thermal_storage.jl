...
function thermal_storage(EP::Model, setup::Dict, inputs::Dict)
	...
	@constraint(EP, cCCAPMax[y in TS], vCCAP[y] <= dfTS[dfTS.R_ID.==y,:Max_Cap_MW_th][])
	...
	@expression(EP, eCFixed_Core[y in TS], dfTS[dfTS.R_ID.==y,:Fixed_Cost_per_MW_th][]*vCCAP[y])
	...
	@constraint(EP, cTSocBalInterior[t in INTERIOR_SUBPERIODS, y in TS], vTS[y,t] ==
		vTS[y,t-1]-(1/dfTS[dfTS.R_ID.==y,:Eff_Therm][]*EP[:vP][y,t])+(dfTS[dfTS.R_ID.==y,:Eff_Up][]*vCP[y,t])-(dfTS[dfTS.R_ID.==y,:Self_Disch][]*vTS[y,t-1]))
	...
	@expression(EP, eCFixed_TS[y in TS], dfTS[dfTS.R_ID.==y,:Fixed_Cost_per_MWh_th][]*vTSCAP[y])
	...
	if !isempty(FUS)
		...
		@constraints(EP, begin
			# Minimum stable core power generated per technology "y" at hour "t" >= Min power of committed core
			[y in TS, t=1:T], vCP[y,t] >= dfTS[dfTS.R_ID.==y,:Min_Power][]*dfTS[dfTS.R_ID.==y,:Cap_Size][]*vFCOMMIT[y,t]

			# Maximum core power generated per technology "y" at hour "t" <= Max power of committed core minus power lost from down time at startup
		...
		for y in FUS
			...
			if Up_Time >= 0
				...
				@constraints(EP, begin
					# Looks back over interior timesteps
					[t in setdiff(INTERIOR_SUBPERIODS,F_Up_Time_HOURS)], vFCOMMIT[y,t] <= sum(vFSHUT[y,e] for e=(t+1-dfTS[dfTS.R_ID.==y,:Up_Time][]):t)

					#wraps around up-time constraint
					[t in F_Up_Time_HOURS], vFCOMMIT[y,t] <= sum(vFSHUT[y,e] for e=(t-((t%hours_per_subperiod)-1):t))+sum(vFSHUT[y,e] for e=((t+hours_per_subperiod-(t%hours_per_subperiod))-(dfTS[dfTS.R_ID.==y,:Up_Time][]-(t%hours_per_subperiod))+1):(t+hours_per_subperiod-(t%hours_per_subperiod)))

					[t in START_SUBPERIODS], vFCOMMIT[y,t] <= vFSHUT[y,t]+sum(vFSHUT[y,e] for e=((t+hours_per_subperiod-1)-(dfTS[dfTS.R_ID.==y,:Up_Time][]-1)+1):(t+hours_per_subperiod-1))
				end)
			end
		end
		...
		#recirculating power from fusion in each zone
