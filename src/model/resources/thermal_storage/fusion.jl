...
function fusion_constraints!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @constraints(EP, begin
        ...
        [t in T, y in FUSION], vCP[t,y] <= core_cap_size(y) * (vCCOMMIT[t,y] -
    ...
    MAX_UPTIME = intersect(FUSION, dfTS[dfTS.Max_Up.>=0, :R_ID])
    ...
    @expression(EP, ePassiveRecircFus[t in T, y in FUSION],
                vCCAP[y] * eff_down(y) * recirc_passive(y))

    # Active recirculating power, depending on committed capacity
    @expression(EP, eActiveRecircFus[t in T, y in FUSION],
...
function total_fusion_power_balance_expressions!(EP::Model, inputs::Dict)
    ...
    @expression(EP, eTotalRecircFus[t in T, y in FUSION],
